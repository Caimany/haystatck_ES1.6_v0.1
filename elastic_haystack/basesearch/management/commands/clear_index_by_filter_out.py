...
def do_update(backend, index, qs, start, end, total, verbosity=1, commit=True):
    ...
    for k in q:
        print(k.pk)
        backend.remove(k.id, commit=True)
        print("removed %s" % (k))
...
class Command(LabelCommand):
    ...
    def update_backend(self, label, using):
        ...
        for model in haystack_get_models(label):
            ...
            # clear_null_index
            if self.workers == 0:
                do_update(backend, index, qs, 0, 0, 0, verbosity=self.verbosity, commit=self.commit)
            else:
                ghetto_queue.append(('do_update', model, 0, 0, 0, using, self.start_date, self.end_date,
