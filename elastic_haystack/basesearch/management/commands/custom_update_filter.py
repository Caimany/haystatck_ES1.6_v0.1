...
class Command(LabelCommand):
    ...
    base_options = (
        ...
        # 过滤字段
        make_option('', '--filed', action='store', dest='filed',
                    default=0, type='str',
                    help=''
                    ),
        make_option('', '--filedkey', action='store', dest='filedkey',
                    default=0, type='str',
                    help=''
                    ),
    ...
    def handle(self, *items, **options):
        ...
        self.filterfiled = options.get('filed', None)
        self.filterkey = options.get('filedkey', None)
    ...
    def update_backend(self, label, using):
        ...
        for model in haystack_get_models(label):
            ...
            queryk = {self.filterfiled:self.filterkey}
            qs = (qs.filter(**queryk))
            total = max_id = qs.count()
