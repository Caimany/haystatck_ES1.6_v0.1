...
from elastic_haystack.settings import Fields_Mapping, Custom_Score
...
try:
    ...
    raise MissingDependency(
        "The 'elasticsearch2' backend requires the installation of 'elasticsearch>=2.0.0,<3.0.0'. Please refer to the documentation.")
...
class Elasticsearch2SearchBackend(BaseSearchBackend):
    ...
    def __init__(self, connection_alias, **connection_options):
        ...
        if 'INDEX_NAME' not in connection_options:
            raise ImproperlyConfigured(
                "You must specify a 'INDEX_NAME' in your settings for connection '%s'." % connection_alias)
        ...
        if filters:
            if not Custom_Score:
                ...
                if len(filters) == 1:
                    kwargs["query"] = {
                        "function_score": {
                            "query": kwargs.pop("query"), "functions": [
                                {
                                    "filter": filters[0],
                                    "script_score": {
                                        "script": Custom_Score
    ...
    def _process_results(self, raw_results, highlight=False,
        ...
        if 'aggregations' in raw_results:
            ...
            for facet_fieldname, facet_info in raw_results['aggregations'].items():
                ...
                if facet_type == 'terms':
                    facets['fields'][facet_fieldname] = [(individual['key'], individual['doc_count']) for individual in
                                                         facet_info['buckets']]
                    ...
                    facets['dates'][facet_fieldname] = [
                        ...
                        individual in facet_info['buckets']]
...
DEFAULT_FIELD_MAPPING = {'type': 'string', 'analyzer': 'index_ansj'}
FIELD_MAPPINGS = {
